...
class TestPaymentOrchestrator:
    ...
    @pytest.fixture
    def invoice(self):
        customer = Customer.objects.create(name="Test Customer", email="test@example.com", phone="1234567890")
        return Invoice.objects.create(payable_amount=100.0, status=InvoiceStatusChoices.CREATED, customer=customer)
    ...
    def test_verify_razorpay_payment_success(self, mock_fetch_payment, mock_verify_signature, orchestrator, invoice):
        ...
        payment = Payment.objects.create(
            invoice=invoice, amount=100.0, order_id="order_123", status=PaymentStatusChoices.CREATED
        )
        ...
        success = orchestrator.verify_razorpay_payment(
            razorpay_order_id="order_123", razorpay_payment_id="pay_123", razorpay_signature="valid_sig"
        )
    ...
    def test_verify_razorpay_payment_failure(self, mock_verify_signature, orchestrator, invoice):
        payment = Payment.objects.create(
            invoice=invoice, amount=100.0, order_id="order_failed", status=PaymentStatusChoices.CREATED
        )

        mock_verify_signature.return_value = False

        success = orchestrator.verify_razorpay_payment(
            razorpay_order_id="order_failed", razorpay_payment_id="pay_failed", razorpay_signature="invalid_sig"
        )
