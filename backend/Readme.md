...
**Clean Code:**
- Keep all the beckend project settings and configurations in `config/` directory.
- Try keeping it clean, if anything is becoming messy break into files/sub-directories, Ex:
    ...
    - As settings could have become messy with variable resolution functions created a `config/settings/resolvers.py`.
...
- Keep the all the application related logic in `project_apps` directory.
