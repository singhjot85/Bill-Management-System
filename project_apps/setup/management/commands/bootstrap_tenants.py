...
from project_apps.utils.tenant_utils import DomainConfig, TenantCreationUtils
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if private_schema := options["schema_name"]:
            self.stdout.write(self.style.SUCCESS(f"Creating Private Tenant: [{private_schema}]..."))
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Error creating private schema: {str(e)}"))
            self.stdout.write(self.style.SUCCESS("Tenant Created Successfully..."))

    def add_arguments(self, parser):
        parser.add_argument(
            "--schema_name", type=str, help="Create a private tenant with schema name along with public tenant"
        )
        parser.add_argument("--domain_name", type=str, help="[Optional] Domain Name for the private tenant.")
